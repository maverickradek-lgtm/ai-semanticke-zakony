...
const SOURCE_META: Record<string, { doc_type: string; issuer: string }> = {
  mf_chj: { doc_type: "metodika", issuer: "MF ČR (CHJ)" },
  cus_podnikatele: { doc_type: "cus_podnikatele", issuer: "Ministerstvo financí ČR – ČÚS" },
  predpis_eu: { doc_type: "predpis_eu", issuer: "EUR-Lex / Rada a Komise EU" },
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const source: string = typeof body?.source === "string" && SOURCE_META[body.source.trim()] ? body.source.trim() : "mf_chj";
    ...
    if (action === "list") {
      ...
      const rows = await sql`
        select d.id, d.title, d.url, d.published_date, d.is_current, d.external_id,
